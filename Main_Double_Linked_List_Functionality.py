...
class BusStreets:
    ...
    #convert python list data into double linked list
    def create_linked_list(self):
        for i in range (len(bus_str)):
            bus_str_LL_temp[i] = DLinkedList()
            for j in range (len(bus_str[i])):
                bus_str_LL_temp[i].addLast(bus_str[i][j])
        bus_str_LL = DLinkedList()
        for i in range (len(bus_str_LL_temp)):
            bus_str_LL.addLast(bus_str_LL_temp[i])
        return bus_str_LL